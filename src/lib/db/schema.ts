...
/* ===========================================================
   ADMIN USERS — session-based auth, single role for v1
   =========================================================== */
...
export const collections = pgTable("collections", {
  ...
  coverImagePublicId: text("cover_image_public_id").default(""), // Cloudinary public_id, for managed deletes
  featuredOnHomepage: boolean("featured_on_homepage").default(false).notNull(),
  sortOrder: integer("sort_order").default(0).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

// Categories are admin-managed too (PRD 2.6 — products assigned to
// categories AND collections), kept as a simple lookup table rather than
// hardcoded so new categories don't require a code change.
...
export const products = pgTable("products", {
  id: serial("id").primaryKey(),
  sku: varchar("sku", { length: 60 }).notNull().unique(), // e.g. CPT/SH/014
  ...
  description: text("description").default(""),
  material: varchar("material", { length: 255 }).default(""),
  ...
  isActive: boolean("is_active").default(true).notNull(), // soft-delete flag — keeps order history intact
  allowCustomRequest: boolean("allow_custom_request").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// One row per size, holding the actual stock count — this is the
// lightweight stock-awareness layer recommended on top of the PRD's
// original "no inventory tracking" scope, so a drop can't be oversold.
export const productVariants = pgTable("product_variants", {
  ...
  productSizeUnique: uniqueIndex("product_variants_product_id_size_idx").on(table.productId, table.size),
}));

/* ===========================================================
   ORDERS — created from the storefront, managed in admin.
   Revenue is only recognized when status = 'delivered' (see
   lib/analytics.ts), matching the PRD's explicit requirement.
   =========================================================== */

export const orderSequences = pgTable("order_sequences", {
  // one row per calendar day (YYYYMMDD), counter incremented atomically
  // inside the same transaction as order creation — this is what avoids
  // the duplicate-order-number race condition flagged in the prototype.
...
export const orders = pgTable("orders", {
  id: serial("id").primaryKey(),
  orderNumber: varchar("order_number", { length: 40 }).notNull().unique(), // CPT/XX/YYYYMMDDNNN
...
export const orderItems = pgTable("order_items", {
  ...
  // Snapshots — protects order history if the product is later edited,
  ...
  customRequestType: varchar("custom_request_type", { length: 60 }).default("None"),
  customRequestNote: text("custom_request_note").default(""),
});

/* ===========================================================
   SHIPPING RATES (by Nigerian state)
   =========================================================== */

export const shippingRates = pgTable("shipping_rates", {
  state: varchar("state", { length: 60 }).primaryKey(),
  fee: numeric("fee", { precision: 12, scale: 2 }).notNull(),
});

/* ===========================================================
   CONTACT MESSAGES (lightweight CRM)
   =========================================================== */
...
/* ===========================================================
   RELATIONS (for ergonomic Drizzle queries with `.with`)
   =========================================================== */
